...
void FlightInsert();	//增加航线       
//修改航线          
//清空系统               //返回主菜单  
//退出系统        
void FlightDelete();	//删除航线
void FlightLookup();	//查询航线
void FlightRework();
...
void FlightRework()   //修改航线信息
{
	...
	else
	{
		while (i != 'c')
		{
			...
			cout << "     *9、头等舱价格    a、普通舱价格*" << endl;
			cout << "     *b、经济舱价格    c、退出      *" << endl;
			cout << "     **********************************" << endl;
			cin >> i;
			switch (i)
			{
			...
			case '8':
				...
				cin >> q->Ration; q->FreeTicket = q->Ration - Customer_Number; 
				break;
			case '9':
				cout << "请输入头等舱价格:" << endl;
				cin >> q->Price[0]; 
...
//
//void Saveplane(PFlight l)	/*保存飞机信息*/
//{
//	FILE* fp;
//	Flight* p;
//	fp = fopen("plane.txt", "wb");
//	if (fp == NULL) {
//		printf("文件无法打开!");
//		return;
//	}
...
//
//
//void Saveman(Customer k){	//保存订票人的信息
//	FILE*fp;
//	Customer *p;
//	fp = fopen("man.txt", "wb");
//	if (fp == NULL) {
...
//	p = k->next;
//	while (p) {
//		if (fwrite(p, sizeof(Customer), 1, fp) == 1) {
//			p = p->next;
//		}
//		else {
//			break;
